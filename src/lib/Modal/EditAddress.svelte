<script lang="ts">
	...
	const handleGetAddress = async () => {
		const response = await fetch(`/api/v1/customer/address?id=${$modalStore[0].meta?.addressID}`, {
			method: 'GET',
			headers: {
		...
		if (response.status === 401) {
			...
			const response = await fetch(
				`/api/v1/customer/address?id=${$modalStore[0].meta?.addressID}`,
				{
					method: 'GET',
					headers: {
						'Content-Type': 'application/json'
	...
	const handlePatchAddAddress = async () => {
		...
		if (response.status === 401) {
			...
			if (response.ok) {
				...
				triggerToast('Address edited successfully', 'variant-filled-primary');
				modalStore.close();
			}
	...
	{#await handleGetAddress() then}
		<div class="card w-full max-w-3xl h-full border border-black max-h-[98%] overflow-y-auto">
			<div class=" px-10 py-6">
				...
				<label class="label">
					<span class="text-sm"
						>Phone Number <span class="badge variant-soft">Required</span>
					</span>
					<input
						type="text"
			...
			<div class="px-10 py-6">
				...
				<label class="label">
					<span class="text-sm"
						>Full Address <span class="badge variant-soft">Required</span>
					</span>
					<input
						type="text"
				...
				<button
					...
					on:click={handlePatchAddAddress}
				>
					{#if isSavingAddress}
